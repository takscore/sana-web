...
export default function CheckoutPage() {
  ...
  if (authLoading) return <p className="p-6 text-gray-500">Loading...</p>;

  if (!user) {
    return (
      <main className="mx-auto max-w-md p-6 text-center">
        <h1 className="mb-4 text-2xl font-bold">Log in to check out</h1>
        <Link href="/login" className="rounded bg-black px-6 py-2 text-white">Log In</Link>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-lg p-6">
      ...
      <div className="mb-6 rounded-lg border bg-white p-4">
        ...
        <div className="mt-2 border-t pt-2 font-bold">Subtotal: MWK {subtotal.toFixed(2)}</div>
      </div>

      <form onSubmit={handlePlaceOrder} className="space-y-4">
        {error && <p className="text-sm text-red-600">{error}</p>}

        <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className="w-full rounded border px-3 py-2" required>
